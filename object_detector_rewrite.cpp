...
void blockMarkerPub(vector<vector<float> > ranges){
	...
	for(int i=0; i<ranges.size(); i++){
    	...
    	//double scale = octomap::OcTreeBaseImpl->getResolution();
    	//double scale = 0.3;
    	marker.scale.x = (ranges[i][1]-ranges[i][0])+0.3;
    	marker.scale.y = (ranges[i][3]-ranges[i][2])+0.3;
    	marker.scale.z = (ranges[i][5]-ranges[i][4])+0.3;
...
int main(int argc, char** argv) {
    ...
    ros::NodeHandle nh;
    ros::Subscriber sub_array=nh.subscribe("/filtered_octo", 1, &objectSegmentation);
    //ros::Subscriber sub_cloud=nh.subscribe("/velodyne_points", 1, &objectFilter);
    ma_pub = nh.advertise<visualization_msgs::MarkerArray>("object_marker_test", 1);
    block_ma_pub = nh.advertise<visualization_msgs::MarkerArray>("block_objects", 1);
    srand (time(0));
    ros::spin();
}